...
def login(user,pwd,KH,OF):
    ...
    if username == "REMARCAR" and password == "2022URRIA!":
        resp, trama_regi = marcar(KH,OF)
        registro_marcado(KH,OF,'NO OFICIAL',trama_regi)
        codigo_textbox.delete(0, tk.END)
        ventana.focus_set()
        codigo_textbox.focus_set()
    elif username == "HORIZONTAL" and password == "EGILE2023":
        try:
            with db_conn_po('ODBC Driver 17 for SQL Server', 'MENITSISTEPLANT', 'marcadongvs', 'escrituraCaptor', '0') as cursor:
...
def registro_marcado(KH,OF,MARCA,trama_regi):
    try:
        with db_conn_po('ODBC Driver 17 for SQL Server', 'MENITSISTEPLANT', 'marcadongvs', 'escrituraCaptor', '0') as cursor:
                    ...
                    print(Registro)
                    consulta = f'INSERT INTO RegistroMarcado (PartNumber, Orden, FechaMarcado, TipoMarcado, ContenidoDM) VALUES (?, ?, ?, ?, ?)'
                    cursor.execute(consulta, KH, OFint, hora, MARCA, trama_regi)
                    cursor.commit()
...
def marcar(KH,OF):
    ...
    try:
        with db_conn_po('ODBC Driver 17 for SQL Server', 'MENITSISTEPLANT', 'marcadongvs', 'escrituraCaptor', '0') as cursor:
            # We truncate the table
            consulta = 'SELECT PartNumber, Fichero, TipoPieza, Marcadora, TipoMarcado FROM ProgramaMarcado WITH (NOLOCK) WHERE PartNumber LIKE ?'
    ...
    if KH in ['KH37831', 'KH37833', 'KH88084', 'KH88086', 'KH88091', 'KH88092']:
        SER = '201293DMP' + OF
    ...
    if KH in ['KH88084', 'KH88086', 'KH88091', 'KH88092']:
        MFR = 'K0680'
        MFR_hex2 = ' '.join([hex(ord(c))[2:].zfill(2) for c in MFR])
        MFR = 'MFR K0680'
    else:
        MFR_hex2 = MFR_hex
    ...
    for i in range(0, 2):
        ...
        if Tipo_Marcado == '1':#tipo de marcado con DM único y rectangular
            ...
            trama = Marcar_Texto + ' ' + Punto + ' 0a ' \
                    ...
                    + PNR_hex + ' 0a ' \
                    + ASSY_hex
            trama_regi = Corchete_regi + RS_regi + '12' + GS_regi + MFR + GS_regi + SER + GS_regi + PNR + RS_regi + EOT_regi
            ...
            DM1 = Corchete + ' ' + RS + ' ' + Doce + ' ' + GS + ' ' + MFR_hex + ' ' + GS + ' ' + SER_hex + ' ' + RS + ' ' + EOT
            DM2 = Corchete + ' ' + RS + ' ' + Doce + ' ' + GS + ' ' + PNR_hex + ' ' + RS + ' ' + EOT
            trama = Marcar_Texto + ' ' + Punto + ' 0a ' \
...
def oficial(coditex):
    ...
    if MAR==ACK:
        ...
        zero()
        registro_marcado(KH,OF,'OFICIAL',trama_regi)
        cursor.close()
