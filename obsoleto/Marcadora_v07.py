...
def open_login_window(KH,OF):
    ...
    button_login = tk.Button(login_window, text="Iniciar sesión", command=login(entry_username.get(),entry_password.get()))
    button_login.place(x=15,y=110)
    button_marcar = tk.Button(login_window, text="Marcar", command=marcar(KH,OF))
    button_marcar.place(x=30,y=110)
    button_marcar.pack_forget()
...
def buscar_y_pegar(valor,archivo):
    ...
    try:
        ventana = pygetwindow.getWindowsWithTitle('Inicio de trabajo OF')
        if len(ventana) == 0:
            
            messagebox.showerror('ERROR', "IDS NO ABIERTO\n1-ABRE EL PROGRAMA [PISTOLA] DEL ESCRITORIO\n2-CLICA LA OPCIÓN [VALIDACIÓN RECEPCIÓN MP]")
            return 0
        else:
            ...
            os.system(f'copy {ruta_destino} {ruta_original}')
            print(f"Imagen guardada como {ruta_destino}")
            time.sleep(0.1)
            # Si existe, seleccionar la ventana y pegar el valor
            ventana[0].activate() # Activar la ventana
        ...
        messagebox.showerror('ERROR', "ERROR EN EL PROGRAMA")
        return 0
...
def oficial(coditex):
    ...
    with db_conn_po('ODBC Driver 17 for SQL Server', 'MENITBD', 'IDSDMP1', 'lecturaIDS', '0') as cursor:
        try:
            ...
            if resultado==1:
                ...
                messagebox.showerror('ERROR', "NÚMERO DE SERIE REPETIDO \n APARTA LA PIEZA Y AVISA A INGENIERÍA")
                #EN EL CASO DE QUE SALGA ESTE ERROR LO QUE HAY QUE HACER ES CAMBIAR EN IDS LA SITUACIÓN DE LA OF QUE NO QUEREMOS MARCAR A CERRADA PARA QUE SOLO QUEDE COMO RESERVADA LA OF QUE QUEREMOS MARCAR
                return
        except:
            messagebox.showerror('ERROR', "NO SE ENCUENTRA EL NÚMERO DE SERIE \n OF NO RESERVADA")
            return
    ...
    IDS=buscar_y_pegar(coditex)#Abre la OF y la Imprime
    if IDS==0:
        return
...
boton_reset = tk.Button(ventana, text='Repetir\nMarcado', width=10, height=2, command=lambda: open_login_window())
boton_reset.place(x=100, y=150)

# Botón de cerrar
boton_cerrar = tk.Button(ventana, text='Cerrar', width=10, height=2, command=ventana.destroy)
boton_cerrar.place(x=200, y=150)

# Botón RESET
boton_reset = tk.Button(ventana, text='Reset\nMarcadora', width=10, height=2, command=lambda: reset_marcadora())
