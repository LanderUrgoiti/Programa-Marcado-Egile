...
def open_login_window(KH,OF):
    ...
    button_login = tk.Button(login_window, text="Iniciar sesión", command=lambda:(login(entry_username.get(),entry_password.get(),KH,OF),login_window.destroy()))
...
def buscar_y_pegar(valor,archivo):
    ...
    try:
        ...
        if len(ventanaIDS) == 0:
            ...
            ventanaIDS[0].activate() # Activar la ventana
...
def oficial(coditex):
    ...
    IDS=buscar_y_pegar(coditex,Codi_Art)#Abre la OF y la Imprime, necesita el SER y el codigo de articulo para el logo de color
    if IDS==0:
        return
