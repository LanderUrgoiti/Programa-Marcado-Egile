    ...
    try:
        with db_conn_po('ODBC Driver 17 for SQL Server', 'MENITSISTEPLANT', 'marcadongvs', 'escrituraCaptor', '0') as cursor:
            ...
            if KH in ['KH18990', 'KH18995']:
                consulta = 'SELECT PartNumber, Fichero FROM ProgramaMarcado WHERE PartNumber LIKE ?'
...
def oficial(coditex):
    ...
    with db_conn_po('ODBC Driver 17 for SQL Server', 'MENITBD', 'IDSDMP1', 'lecturaIDS', '0') as cursor:
        ...
        consulta = 'SELECT XLIPA.Descripcion FROM XLIPA INNER JOIN ORDEN ON XLIPA.NumeroIPA = ORDEN.NumeroIPA AND XLIPA.Nivel = ORDEN.NivelIPA AND XLIPA.Revision = ORDEN.RevisionIPA WHERE ORDEN.Numero LIKE ?'
