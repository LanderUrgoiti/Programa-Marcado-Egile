...
def buscar_y_pegar(Codigo):
    #FUNCIÓN PARA MIRAR DI IDS ESTÁ ABIERTO Y PEGAR EL NÚMERO DE SERIE DE FUNDIDO SIN EL SER
    NombreVentana='Inicio de trabajo OF'
    ventana = pygetwindow.getWindowsWithTitle(NombreVentana)
    if len(ventana) == 0:
        '''
        # Si no existe, ejecutar el programa correspondiente
        pyautogui.press('win') # Simular presionar la tecla de Windows
        time.sleep(0.1)
        pyautogui.typewrite('Bloc de notas') # Escribir el nombre del programa
        pyautogui.press('enter') # Simular presionar la tecla Enter
        time.sleep(0.5)
        pyperclip.copy(Codigo)
        pyautogui.hotkey('ctrl', 'v') # Pegar el valor
        print('No hay ventana')
        '''
        messagebox.showerror('ERROR', "IDS NO ESTÁ ABIERTO \n ÁBRELO Y ESCANEA DE NUEVO")
        return
...
def marcar(coditex):
    ...
    client_socket.close()
    estado()
    buscar_y_pegar(coditex)#Abre la OF y la Imprime
...
'''
    i=0
    while i<30 and not RESPUESTA_MARCHA in hex_string:
        #response_bytes = []
        response_bytes = client_socket.recv(1024)
        hex_string = ' '.join(['{:02x}'.format(byte) for byte in response_bytes])
        time.sleep(1)
        i=i+1
        print(i)
    if hex_string == RESPUESTA_MARCHA:
        resultado_label2.config(text='Pieza Marcada')
        print('Pieza marcada')
    else:
        messagebox.showerror('ERROR', "NO HAY RESPUESTA DE LA MARCADORA")
        return
'''
